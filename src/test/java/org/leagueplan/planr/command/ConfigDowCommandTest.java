...
class ConfigDowCommandTest extends CommandTestBase {
    ...
    class Set {
        ...
        void successWithFullName() {
            int exit = execute("config", "dow", "set",
        ...
        void failsOnUnrecognizedDay() {
            int exit = execute("config", "dow", "set",
                "--day", "funday", "--start", "09:00", "--end", "18:00");
            assertEquals(1, exit);
        ...
        void failsOnInvalidStartTime() {
            int exit = execute("config", "dow", "set",
                "--day", "monday", "--start", "9am", "--end", "18:00");
            assertEquals(1, exit);
        ...
        void failsOnInvalidEndTime() {
            int exit = execute("config", "dow", "set",
                "--day", "monday", "--start", "09:00", "--end", "6pm");
            assertEquals(1, exit);
            assertTrue(stderr().contains("Invalid time"));
        }
        ...
        void failsWhenEndEqualsStart() {
            int exit = execute("config", "dow", "set",
                "--day", "monday", "--start", "09:00", "--end", "09:00");
            assertEquals(1, exit);
        ...
        void failsWhenEndBeforeStart() {
            int exit = execute("config", "dow", "set",
                "--day", "monday", "--start", "18:00", "--end", "09:00");
            assertEquals(1, exit);
            assertTrue(stderr().contains("after"));
        }

        @Test
        @DisplayName("emits singular 'entry' warning when exactly one field-level entry exists on that day")
        void emitsSingularConflictWarning() {
            ...
            execute("field", "block", "add", "Riverside Park",
                "--date", "2026-06-03", "--start", "10:00", "--end", "12:00");
        ...
        @Test
        @DisplayName("emits plural 'entries' warning when multiple field-level entries exist on that day")
        ...
        void noWarningWhenNoConflicts() {
            ...
            execute("field", "block", "add", "Riverside Park",
                "--date", "2026-06-04", "--start", "10:00", "--end", "12:00");
        ...
        void conflictWarningIsScopedToSeason() {
            ...
            execute("field", "block", "add", "Riverside Park",
                "--date", "2026-05-31", "--start", "10:00", "--end", "12:00");

            execute("config", "dow", "set", "--day", "sunday", "--start", "09:00", "--end", "12:00");

            assertFalse(stdout().contains("Warning"),
                "block outside season should not trigger warning");
        }
        ...
        void allEntriesCountedWhenNoSeasonConfigured() {
            ...
            assertTrue(stdout().contains("Warning"),
                "should warn even without a season when entries exist");
        }

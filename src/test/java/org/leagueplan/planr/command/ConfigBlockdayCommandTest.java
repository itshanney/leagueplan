...
class ConfigBlockdayCommandTest extends CommandTestBase {
  ...
  class Add {
    ...
    @Test
    @DisplayName(
        "emits singular 'entry' conflict warning when exactly one field-level entry exists on that"
            + " day")
    ...
    @Test
    @DisplayName(
        "conflict warning mentions that FieldDateOverride entries take precedence over the block")
    void conflictWarningMentionsPrecedence() {
      ...
      assertTrue(out.contains("Warning"));
      assertTrue(
          out.contains("FieldDateOverride"), "warning should mention FieldDateOverride precedence");
    }

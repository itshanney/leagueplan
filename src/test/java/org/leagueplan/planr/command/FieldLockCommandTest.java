...
class FieldLockCommandTest extends CommandTestBase {
  ...
  class Delete {
    ...
    @DisplayName("exits 0 and prints confirmation on success")
    void success() {
      int exit = execute("field", "lock", "delete", "--field", "Riverside Park", "--index", "1");
      assertEquals(0, exit);
      String out = stdout();
    ...
    @DisplayName("exits 1 when field does not exist")
    void failsWhenFieldNotFound() {
      int exit = execute("field", "lock", "delete", "--field", "NonExistent", "--index", "1");
      assertEquals(1, exit);
      assertTrue(stderr().contains("not found"));
    }
    ...
    void failsWhenIndexOutOfRange() {
      int exit = execute("field", "lock", "delete", "--field", "Riverside Park", "--index", "99");
      assertEquals(1, exit);
      assertTrue(stderr().contains("not found"));
    }
    ...
    void failsWhenNoLocksExist() {
      ...
      int exit = execute("field", "lock", "delete", "--field", "Empty Field", "--index", "1");
      assertEquals(1, exit);

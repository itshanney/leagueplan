 ...
 *
 * Excludes `practice assign` (requires the CP-SAT solver + stdin interaction).
 * All tests use 2-team divisions with small practice counts (1-2) to stay fast.
 *
 * Setup pattern:
 *   - Add division with --practice-count, --practice-duration-minutes, --practice-start, --practice-end
 *   - Add teams
 *   - Add field + config for assign tests only
 */
class PracticeCommandTest extends CommandTestBase {
    ...
    private void addConfiguredDivision(String name, int practiceCount) {
        ...
        execute("division", "edit", name,
            "--practice-count", String.valueOf(practiceCount),
            "--practice-duration-minutes", "60",
            "--practice-start", PRAC_START,
            "--practice-end", PRAC_END);

...
/**
 * Tests for practice-related flags on `planr division edit` and the new columns in `planr division
 * list`.
 *
 * <p>These flags were added in v0.10.0: --practice-count <n> --practice-duration-minutes <n>
 * --practice-start <YYYY-MM-DD> --practice-end <YYYY-MM-DD>
 *
 * <p>Season configured at 2026-06-01 so practice-start/end validation (must be < seasonStart) has a
...
class DivisionCommandPracticeTest extends CommandTestBase {
  ...
  class PracticeDurationMinutes {
    ...
    void persistsPracticeDuration() {
      addDivision("Majors");
      int exit = execute("division", "edit", "Majors", "--practice-duration-minutes", "90");
      assertEquals(0, exit);
    ...
    void failsOnZeroDuration() {
      addDivision("Majors");
      int exit = execute("division", "edit", "Majors", "--practice-duration-minutes", "0");
      assertEquals(1, exit);
      assertTrue(stderr().contains("positive integer"));
    }
    ...
    void failsOnNegativeDuration() {
      addDivision("Majors");
      int exit = execute("division", "edit", "Majors", "--practice-duration-minutes", "-30");
      assertEquals(1, exit);
      assertTrue(stderr().contains("positive integer"));
    }
  }
  ...
  class PracticeDates {
    ...
    void failsWhenStartOnOrAfterSeasonStart() {
      ...
      int exit =
          execute(
              ...
              "Majors",
              "--practice-start",
              SEASON_START); // same day as seasonStart
      assertEquals(1, exit);
    ...
    void failsOnInvalidStartDate() {
      addDivision("Majors");
      int exit = execute("division", "edit", "Majors", "--practice-start", "not-a-date");
      assertEquals(1, exit);
    ...
    void failsOnInvalidEndDate() {
      addDivision("Majors");
      int exit = execute("division", "edit", "Majors", "--practice-end", "2026/05/15");
      assertEquals(1, exit);
      assertTrue(stderr().contains("Invalid date"));
    }
    ...
    void canSetStartAloneWithNoExistingEnd() {
      ...
      int exit = execute("division", "edit", "Majors", "--practice-start", "2026-04-01");
      assertEquals(0, exit);
    }
    ...
    void validatesNewStartAgainstStoredEnd() {
      ...
      int exit =
          execute(
              ...
              "Majors",
              "--practice-start",
              "2026-05-15"); // after stored end 2026-05-01
      assertEquals(1, exit);
      assertTrue(stderr().contains("must not be before"));
    }
  }
  ...
  class ListColumns {
    ...
    void showsDashForUnconfiguredFields() {
      ...
      long dashCount = java.util.Arrays.stream(stdout().split("\\s+")).filter("--"::equals).count();

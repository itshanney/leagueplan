 ...
 *
 * <p>Bracket size P = next power of 2 ≥ N. B = P - N bye slots are assigned to seeds 1..B in
 * Winners R1. Seeds B+1..N face each other in Winners R1. Subsequent rounds use positional
 * references ("W of G3", "L of G2").
 *
 * <p>Slot counts: Winners bracket: P - 1 slots (including B bye slots) Losers bracket: P - 2 slots
 * (no byes in losers) Championship: 2 slots (final + conditional re-match) Total real game slots:
 * 2N - 1 + 1 conditional = 2N Total with byes: 2N + B
 */
public class PlayoffBracketService {
  ...
  /**
   * Generates all bracket slots for the given seed list. seeds[0] is seed 1 (top seed), seeds[N-1]
   * is seed N.
   */
  public List<BracketSlot> generateBracket(List<String> seeds) {
    ...
    for (int i = 0; i < p / 2; i++) {
      ...
      slots.add(
          new BracketSlot(gameId, "Winners R1", BracketSide.WINNERS, posA, posB, false, isBye));
    }
    ...
    List<UUID> realWinnersR1 = winnersR1Ids.stream().filter(id -> !isById(slots, id)).toList();
    ...
    while (winnersFeedIndex < winnersRoundDropIds.size() || prevLosers.size() > 1) {
      if (winnersFeedIndex < winnersRoundDropIds.size()) {
        ...
        for (int i = 0; i < droppers.size(); i++) {
          ...
          String posA = i < prevLosers.size() ? prevLosers.get(i).label(slots) : "TBD";
          slots.add(
              new BracketSlot(
                  gameId,
                  "Losers R" + losersRound,
  ...
  private static boolean isById(List<BracketSlot> slots, UUID id) {
    return slots.stream()
        .filter(s -> s.gameId().equals(id))
        .findFirst()
        .map(BracketSlot::isBye)
        .orElse(false);
  }

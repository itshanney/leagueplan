...
public class FieldBlockCommand implements Runnable {
  ...
  static void printInvalidBlockError(Field field, int blockNumber) {
    if (field.blocks().isEmpty()) {
      System.err.printf(
          "Error: Block #%d not found for \"%s\" (no blocks exist).%n", blockNumber, field.name());
    } else {
      System.err.printf(
          "Error: Block #%d not found for \"%s\" (1–%d are valid).%n",
  ...
  static class AddCmd implements Callable<Integer> {
    ...
    public Integer call() {
      ...
      if (start.isEmpty()) {
        System.err.printf(
            "Error: Invalid time \"%s\". Use HH:mm format (e.g., 09:00).%n", startStr);
        return 1;
      }
  ...
  static class EditCmd implements Callable<Integer> {
    ...
    public Integer call() {
      ...
      try {
        ...
        FieldBlock replacement =
            new FieldBlock(existing.id(), effectiveDate, effectiveStart, effectiveEnd);
  ...
  static class ListCmd implements Callable<Integer> {
    ...
    public Integer call() {
      try {
        ...
        if (field.blocks().isEmpty()) {
          System.out.printf(
              "No blocks for \"%s\". Use 'planr field block add' to create one.%n", field.name());
          return 0;
        }
        printTable(field);
        return 0;

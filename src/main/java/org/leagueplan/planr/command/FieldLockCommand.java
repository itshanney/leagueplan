...
@Command(
    name = "lock",
    description =
        "Manage field division locks (restrict a field to one division for a date range).",
    subcommands = {
...
public class FieldLockCommand implements Runnable {
  ...
  static class AddCmd implements Callable<Integer> {
    ...
    @Option(names = "--field", required = true, paramLabel = "<name>", description = "Field name.")
    String fieldName;
    ...
    public Integer call() {
      ...
      if (end.get().isBefore(start.get())) {
        System.err.printf(
            "Error: End date %s must not be before start date %s.%n", end.get(), start.get());
        return 1;
      }
      try {
        ...
        if (!conflicts.isEmpty()) {
          String indices = conflicts.stream().map(n -> "#" + n).collect(Collectors.joining(", "));
          System.err.printf(
              "Error: Lock date range overlaps with existing lock(s): %s. "
                  + "Use 'planr field lock list' to review.%n",
              indices);
          return 1;
        }
  ...
  static class ListCmd implements Callable<Integer> {
    ...
    public Integer call() {
      try {
        ...
        rows.sort(Comparator.comparing(LockRow::fieldName).thenComparing(LockRow::startDate));
        ...
        int numW =
            Math.max(
                1,
                rows.stream()
                    .mapToInt(r -> String.valueOf(r.lockNumber()).length())
                    .max()
                    .orElse(0));
        int divW =
            Math.max(
        ...
        String fmt =
            "%-" + fieldW + "s  %-" + numW + "s  %-" + divW + "s  %-" + dateW + "s  %-" + dateW
                + "s%n";
        ...
        for (LockRow r : rows) {
          System.out.printf(
              fmt, r.fieldName(), r.lockNumber(), r.divisionName(), r.startDate(), r.endDate());
        }
        return 0;
      } catch (IOException e) {
        System.err.printf("Error: Failed to access league data: %s%n", e.getMessage());

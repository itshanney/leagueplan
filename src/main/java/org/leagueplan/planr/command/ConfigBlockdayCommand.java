    ...
    public Integer call() {
      ...
      try {
        ...
        if (config.blockedDays().contains(day.get())) {
          System.err.printf(
              "Error: %s is already a blocked day.%n", DayParser.displayName(day.get()));
          return 1;
        }
        LeagueConfig updated = config.withBlockedDayAdded(day.get());
        parent.configCmd.app.store.save(league.withConfig(updated));
        System.out.printf("%s added to blocked days.%n", DayParser.displayName(day.get()));
  ...
  static class RemoveCmd implements Callable<Integer> {
    ...
    public Integer call() {
      ...
      try {
        ...
        LeagueConfig config = (league.config() != null) ? league.config() : LeagueConfig.empty();
        if (!config.blockedDays().contains(day.get())) {
          System.err.printf("Error: %s is not a blocked day.%n", DayParser.displayName(day.get()));
          return 1;
        }
        LeagueConfig updated = config.withBlockedDayRemoved(day.get());
        parent.configCmd.app.store.save(league.withConfig(updated));
        System.out.printf("%s removed from blocked days.%n", DayParser.displayName(day.get()));
        return 0;
      } catch (IOException e) {
        System.err.printf("Error: Failed to access league data: %s%n", e.getMessage());
        return 2;

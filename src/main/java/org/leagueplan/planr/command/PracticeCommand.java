...
public class PracticeCommand implements Runnable {
            ...
            try {
                ...
                for (Division division : league.divisions()) {
                    if (!division.isPracticeConfigured()) {
                        System.out.printf(
                            "Skipping %s: practice configuration incomplete.%n", division.name());
                        continue;
                    }
                    ...
                    if (division.teams().isEmpty()) {
                        System.out.printf(
                            "Skipping %s: no teams configured.%n", division.name());
                        continue;
                    }
                    ...
                    for (Team team : division.teams()) {
                        for (int p = 1; p <= practiceCount; p++) {
                            slots.add(new PracticeSlot(
                                UUID.randomUUID(), team.id(), p, null, null, null));
                        }
                    }

                    PracticeSchedule ps = new PracticeSchedule(
                        division.id(), PracticeState.GENERATED, slots);
                    league = league.withPracticeScheduleAdded(ps);

                    System.out.printf("Generated %d practice slots for %s (%d teams × %d practices).%n",
                ...
                if (divisionsProcessed == 0) {
                    System.err.println("Error: No divisions qualify for practice generation. "
    ...
    static class AssignCmd implements Callable<Integer> {
        ...
        public Integer call() {
            try {
                ...
                int totalSlots = schedules.stream()
                    .mapToInt(ps -> ps.slots().size())
                    .sum();
                ...
                System.out.printf("Practice field assignment: %d slot(s) across %d division(s).%n",
                ...
                System.out.print(
                    "Begin field assignment? This may take up to 5 minutes. "
                ...
                for (PracticeSchedule ps : schedules) {
                    List<PracticeSlot> clearedSlots = ps.slots().stream()
                        .map(PracticeSlot::withAssignmentCleared)
                        .toList();
                    cleared = cleared.withPracticeScheduleReplaced(
                        ps.divisionId(), ps.withSlots(clearedSlots));
                }

                System.out.printf("[0:00] Practice field assignment started. "
                    + "%d slot(s) across %d division(s).%n", totalSlots, schedules.size());
                System.out.flush();
                ...
                for (PracticeSchedule ps : cleared.practiceSchedules()) {
                    List<PracticeSlot> updatedSlots = ps.slots().stream()
                        .map(slot -> {
                            var slotResult = success.assignmentsBySlotId().get(slot.slotId());
                            return slotResult != null
                                ? slot.withAssignment(slotResult.date(), slotResult.startTime(),
                                    slotResult.fieldId())
                                : slot;
                        })
                        .toList();
                    updated = updated.withPracticeScheduleReplaced(
                        ps.divisionId(),
                        ps.withSlots(updatedSlots).withState(PracticeState.ASSIGNED));
                }
    ...
    static class StatusCmd implements Callable<Integer> {
            ...
            try {
                ...
                if (psOpt.isEmpty()) {
                    System.err.printf(
                        ...
                        + "Run 'planr practice generate' first.%n", division.name());
                    return 1;
                }

                PracticeSchedule ps = psOpt.get();
                System.out.printf("Division: %s | State: %s | Period: %s to %s%n%n",
                    division.name(), ps.state(),
                    division.practiceStart(), division.practiceEnd());
        ...
        private void printSummary(League league) {
            List<Division> all = league.divisions().stream()
            ...
            int divW = Math.max("DIVISION".length(),
                all.stream().mapToInt(d -> d.name().length()).max().orElse(0));
            ...
            System.out.printf(fmt,
                "-".repeat(divW), "-".repeat(stateW), "-".repeat(assignedW), "-".repeat(totalW));
            ...
            for (Division div : all) {
                ...
                if (psOpt.isPresent()) {
                    PracticeSchedule ps = psOpt.get();
                    state    = ps.state().name();
                    assigned = String.valueOf(
                        ps.slots().stream().filter(s -> s.assignedDate() != null).count());
        ...
        private void printDetail(PracticeSchedule ps, Division division, League league) {
            ...
            String fmt = "%-" + teamW + "s  %-" + pracW + "s  %-" + dateW + "s  %-"
                + timeW + "s  %-" + fieldW + "s%n";
            System.out.printf(fmt, "TEAM", "PRACTICE", "DATE", "TIME", "FIELD");
            System.out.printf(fmt,
                "-".repeat(teamW), "-".repeat(pracW), "-".repeat(dateW),
                "-".repeat(timeW), "-".repeat(fieldW));
    ...
    static class ClearCmd implements Callable<Integer> {
        ...
        public Integer call() {
            try {
                ...
                if (psOpt.isEmpty()) {
                    System.err.printf(
                        "Error: No practice schedule exists for \"%s\".%n", division.name());
                    return 1;
                }

                PracticeSchedule ps = psOpt.get();
                long assigned = ps.slots().stream()
                    .filter(s -> s.assignedDate() != null).count();

...
public class FieldOverrideCommand implements Runnable {
  ...
  static void printInvalidOverrideError(Field field, int overrideNumber) {
    if (field.dateOverrides().isEmpty()) {
      System.err.printf(
          "Error: Override #%d not found for \"%s\" (no overrides exist).%n",
          overrideNumber, field.name());
    } else {
      System.err.printf(
          "Error: Override #%d not found for \"%s\" (1–%d are valid).%n",
  ...
  static void printSeasonWarning(League league, LocalDate date) {
    ...
    if (date.isBefore(config.seasonStart()) || date.isAfter(config.seasonEnd())) {
      System.out.printf(
          "Warning: Override date %s is outside the configured season (%s to %s). Override"
              + " saved.%n",
  ...
  static class AddCmd implements Callable<Integer> {
    ...
    public Integer call() {
      ...
      if (start.isEmpty()) {
        System.err.printf(
            "Error: Invalid time \"%s\". Use HH:mm format (e.g., 10:00).%n", startStr);
        return 1;
      }
      ...
      try {
        ...
        FieldDateOverride override =
            new FieldDateOverride(UUID.randomUUID(), date.get(), start.get(), end.get());
        ...
        System.out.printf(
            "Override #%d added to \"%s\" (%s, open %s–%s).%n",
            overrideNumber,
            field.name(),
            date.get(),
  ...
  static class EditCmd implements Callable<Integer> {
    ...
    public Integer call() {
      if (newDateStr == null && newStartStr == null && newEndStr == null) {
        System.err.println("Error: At least one of --date, --start, or --end must be provided.");
        return 1;
      }
      ...
      try {
        ...
        FieldDateOverride replacement =
            new FieldDateOverride(existing.id(), effectiveDate, effectiveStart, effectiveEnd);

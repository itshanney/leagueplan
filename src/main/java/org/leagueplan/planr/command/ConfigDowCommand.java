...
public class ConfigDowCommand implements Runnable {
    ...
    /**
     * Counts FieldBlock and FieldDateOverride entries on dates matching {@code day}.
     * Scoped to [seasonStart, seasonEnd] when configured; otherwise counts all entries.
     */
    static int countConflicts(League league, DayOfWeek day) {
        ...
        for (Field field : league.fields()) {
            for (var block : field.blocks()) {
                if (block.date().getDayOfWeek() == day
                        && inSeason(block.date(), seasonStart, seasonEnd)) {
                    count++;
                }
            }
    ...
    static class SetCmd implements Callable<Integer> {
        ...
        public Integer call() {
            ...
            if (start.isEmpty()) {
                System.err.printf("Error: Invalid time \"%s\". Use HH:mm format (e.g., 16:00).%n",
                    startStr);
                return 1;
            }
            Optional<LocalTime> end = parseTime(endStr);
            if (end.isEmpty()) {
                System.err.printf("Error: Invalid time \"%s\". Use HH:mm format (e.g., 21:00).%n",
                    endStr);
                return 1;
            }
            if (!end.get().isAfter(start.get())) {
                System.err.printf("Error: End time (%s) must be after start time (%s).%n",
                    endStr, startStr);
                return 1;
            }
            try {
                League league = parent.configCmd.app.store.load();
                LeagueConfig existing = (league.config() != null)
                    ? league.config() : LeagueConfig.empty();
                LeagueConfig updated = existing.withDowWindowSet(
                    new DayOfWeekWindow(day.get(), start.get(), end.get()));
                parent.configCmd.app.store.save(league.withConfig(updated));
                System.out.printf("Day-of-week window set: %s %s–%s.%n",
        ...
        public Integer call() {
            ...
            try {
                ...
                LeagueConfig config = (league.config() != null)
                    ? league.config() : LeagueConfig.empty();
                boolean exists = config.dowWindows().stream()
                    .anyMatch(w -> w.day() == day.get());
                if (!exists) {
                    System.err.printf("Error: No day-of-week window configured for %s.%n",
                        DayParser.displayName(day.get()));
                    return 1;
                }
                LeagueConfig updated = config.withDowWindowRemoved(day.get());
                parent.configCmd.app.store.save(league.withConfig(updated));
                System.out.printf("Day-of-week window for %s removed.%n",
                    DayParser.displayName(day.get()));
                return 0;
            } catch (IOException e) {
                System.err.printf("Error: Failed to access league data: %s%n", e.getMessage());
    ...
    static class ListCmd implements Callable<Integer> {
        ...
        private void printTable(LeagueConfig config) {
            ...
            config.dowWindows().stream()
                ...
                .forEach(w -> System.out.printf(fmt,
